...
class DynamicSpecializationModelClass(EconModelClass):
    ...
    def allocate(self):
        ...
        # c. solution
        shape = (par.T,par.num_K,par.num_K)
        ...
        for t in reversed(range(par.T)):
            ...
            for i_kf,capital_f in enumerate(par.grid_K):
                for i_km,capital_m in enumerate(par.grid_K):
                    idx = (t,i_kf,i_km)
                    idx_last = (t+1,i_kf,i_km)
                    
                    # d. find optimal time-allocation at this level of human capital.
                    obj = lambda x: self.value_of_choice(x,capital_f,capital_m,V_next)  
    ...
    def value_of_choice(self,x,capital_f,capital_m,V_next=None):
        ...
        # c. current utility
        utility = self.util(labor_f,home_f,labor_m,home_m,capital_f,capital_m)
        
        # c. continuation value
        if V_next is not None:
            kf_next = self.human_capital_next(capital_f,labor_f)
            km_next = self.human_capital_next(capital_m,labor_m)
            V_interp = interp_2d(par.grid_K,par.grid_K,V_next,kf_next,km_next)
        else:
            V_interp = 0.0
    ...
    def util(self,labor_f,home_f,labor_m,home_m,capital_f,capital_m):
        # a. unpack
        par = self.par
