...
class DynHouseholdLaborModelClass(EconModelClass):
    ...
    def consumption(self,hours1,hours2,capital1,capital2):
        ...
        tax_hh = self.tax_func(income1,income2)
    ...
    def tax_func(self,income1,income2):
        par = self.par
        
        if par.joint_tax:
            tax = (1.0 - par.tax_scale*((income1+income2)**(-par.tax_pow)))*(income1 + income2)
        else:
            tax1 = (1.0 - par.tax_scale_indiv*((income1)**(-par.tax_pow_indiv)))*(income1)
            tax2 = (1.0 - par.tax_scale_indiv*((income2)**(-par.tax_pow_indiv)))*(income2)
            tax = tax1 + tax2
    ...
    def util(self,hours1,hours2,capital1,capital2):
        ...
        util_cons1 = par.mu * (cons/2)**(1.0+par.eta) / (1.0+par.eta)
        util_cons2 = (1.0-par.mu) * (cons/2)**(1.0+par.eta) / (1.0+par.eta)
        util_hours1 = par.mu * par.rho_1*(hours1)**(1.0+par.gamma) / (1.0+par.gamma)
        util_hours2 = (1.0 - par.mu) * par.rho_2*(hours2)**(1.0+par.gamma) / (1.0+par.gamma)

        return util_cons1 + util_cons2 - util_hours1 - util_hours2
